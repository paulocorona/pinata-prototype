...
/** How long rested candy stays on the courtyard before fading. */
const GROUND_LIFE = 3.2;
...
export class CandyBurst {
  ...
  rain(origin: THREE.Vector3, count: number): void {
    const n = Math.min(count, BASE.particleCap - this.active.length);
  ...
  update(dt: number, arena?: Arena): void {
    for (let i = this.active.length - 1; i >= 0; i--) {
      ...
      if (p.grounded || !p.collide) p.life -= dt;
      if (!p.grounded) {
        p.vel.y -= 9.5 * dt;
      ...
      } else if (p.grounded && p.life < 0.45) {
        p.mesh.scale.setScalar(p.baseScale * Math.max(0.01, p.life / 0.45));
      } else {
        p.mesh.scale.setScalar(p.baseScale);
      }

      const fallenOff = p.mesh.position.y < -1;
      if (p.life <= 0 || fallenOff) {
        p.mesh.visible = false;
        p.active = false;
        this.pool.release(p);
